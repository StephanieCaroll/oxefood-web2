...
export default function FormLogin() {
  ...
  function entrar() {
    if (username !== "" && senha !== "") {
      ...
      axios
        .post("http://localhost:8080/api/auth", authenticationRequest) //Sempre vai armazenar o token
        .then((response) => {
          registerSuccessfulLoginForJwt( 
            response.data.token, //token
            response.data.expiration //expiração do token
          );
          navigate("/home"); 
        })
        .catch((error) => {
    ...
    <div>
      ...
      >
        <Grid.Column style={{ maxWidth: 500 }}>
          <center>
            {" "}
            <Image src="/logo-IFPE.png" size="medium" />{" "}
          </center>
