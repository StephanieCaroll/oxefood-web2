...
export default function ListEnderecos() {
  ...
  function carregarEnderecosDoCliente(id) {
    axios
      .get(`http://localhost:8080/api/cliente/${id}/endereco`)
      .then((response) => {
        setListaEnderecos(response.data);
      })
      .catch((error) => {
  ...
  async function remover() {
    try {
      await axios.delete(
        `http://localhost:8080/api/cliente/endereco/${idRemover}`
      );
    ...
    <div>
      ...
      <div style={{ marginTop: "3%" }}>
        <Container textAlign="justified">
          <h2>
            Endereços {nomeCliente && `de ${nomeCliente}`}{" "}
          ...
          <div style={{ marginTop: "4%" }}>
            <Button
              ...
              to={
                clienteId
                  ? `/form-endereco?clienteId=${clienteId}`
                  : "/form-endereco"
              }
            />
